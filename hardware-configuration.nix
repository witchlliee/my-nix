# Do not modify this file!  It was generated by ‘nixos-generate-config’
# and may be overwritten by future invocations.  Please make changes
# to /etc/nixos/configuration.nix instead.
{ config, lib, pkgs, modulesPath, ... }:

{
  imports =
    [ (modulesPath + "/installer/scan/not-detected.nix")
    ];

  boot.initrd.availableKernelModules = [ "xhci_pci" "ahci" "nvme" "usbhid" "usb_storage" "sd_mod" ];
  boot.initrd.kernelModules = [ ];
  boot.kernelModules = [ ];
  boot.extraModulePackages = [ ];

  fileSystems."/" =
    { device = "/dev/disk/by-uuid/2bc76c12-6a92-4afa-b87f-da8cb077c7c0";
      fsType = "btrfs";
      options = [ "subvol=@" "defaults" "noatime" "compress=zstd" "commit=120" ];
    };

  fileSystems."/home" =
    { device = "/dev/disk/by-uuid/2bc76c12-6a92-4afa-b87f-da8cb077c7c0";
      fsType = "btrfs";
      options = [ "subvol=@home" "defaults" "noatime" "compress=zstd" "commit=120" ];
    };

  fileSystems."/nix" =
    { device = "/dev/disk/by-uuid/2bc76c12-6a92-4afa-b87f-da8cb077c7c0";
      fsType = "btrfs";
      options = [ "subvol=@nix" "defaults" "noatime" "compress=zstd" "commit=120" ];
    };

  fileSystems."/persist" =
    { device = "/dev/disk/by-uuid/2bc76c12-6a92-4afa-b87f-da8cb077c7c0";
      fsType = "btrfs";
      options = [ "subvol=@persist" "defaults" "noatime" "compress=zstd" "commit=120" ];
    };

  fileSystems."/var/log" =
    { device = "/dev/disk/by-uuid/2bc76c12-6a92-4afa-b87f-da8cb077c7c0";
      fsType = "btrfs";
      options = [ "subvol=@log" "defaults" "noatime" "compress=zstd" "commit=120" ];
    };

  fileSystems."/var/cache" =
    { device = "/dev/disk/by-uuid/2bc76c12-6a92-4afa-b87f-da8cb077c7c0";
      fsType = "btrfs";
      options = [ "subvol=@cache" "defaults" "noatime" "compress=zstd" "commit=120" ];
    };

   fileSystems."/boot" =
    { device = "/dev/disk/by-uuid/84F3-558E";
      fsType = "vfat";
      options = [ "fmask=0022" "dmask=0022" ];
    };

   
  swapDevices = [ ];

  # Enables DHCP on each ethernet and wireless interface. In case of scripted networking
  # (the default) this is the recommended approach. When using systemd-networkd it's
  # still possible to use this option, but it's recommended to use it in conjunction
  # with explicit per-interface declarations with `networking.interfaces.<interface>.useDHCP`.
  networking.useDHCP = lib.mkDefault true;
  # networking.interfaces.eno1.useDHCP = lib.mkDefault true;

  nixpkgs.hostPlatform = lib.mkDefault "x86_64-linux";
  hardware.cpu.intel.updateMicrocode = lib.mkDefault config.hardware.enableRedistributableFirmware;
}
